...
import type { Expense, FinanceCategory } from "@/features/finance/types";
  ...
  return (
    <div className="rounded-xl border bg-white overflow-hidden">
      <div className="px-4 py-3 border-b flex items-center justify-between">
        <div>
          <div className="text-sm font-semibold text-gray-900">Ausgaben (Tabelle)</div>
          <div className="text-xs text-gray-600">Excel-Style Übersicht für den Monat</div>
        </div>
      </div>
              ...
              sorted.map((r) => (
                  <tr key={r.id} className="border-t">
                    ...
                    <td className="px-4 py-2">
                      {categories.find((c) => c.id === r.categoryId)?.name ?? "Unbekannt"}
                    </td>
